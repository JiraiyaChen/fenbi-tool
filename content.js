...
(function () {
  ...
  // requestAnimationFrame 节流函数（用于 scroll/resize）
  ...
  const debouncedScan = debounce(function () {
    ...
    document.querySelectorAll('app-result-common').forEach(processResultCommon);
  }, 300);
  ...
  function addVideoControls(videoElement) {
    ...
    const tryAddPanel = () => {
      ...
      // 用 rafThrottle 节流 scroll/resize，避免高频重排
      const throttledUpdate = rafThrottle(updatePanelPosition);
      ...
      // 用 MutationObserver 替代 setInterval 检测视频元素被移除
